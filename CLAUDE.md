...
- **Typing `/plan` gets native plan mode, not the skill.** `plan` is a built-in slash command in the client, so the keystroke renders there and never reaches the agent; the operator's entry is bare prose — `plan: <task>`, or just the task, which the bullet above already routes to the skill. Landing in plan mode anyway costs one operator approval to leave, and `@.claude/skills/plan/SKILL.md` § "If the session is already in native plan mode" owns the recovery.
