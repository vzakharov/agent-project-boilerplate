...
**A comment body is text, not a path to text.** Claude Code expands `@<path>` in a prompt; GitHub does not, and nobody in the thread can reach your disk — so a reply posted as `@/tmp/…/scratchpad/reply.md` lands as that string and nothing else. Draft into a file freely, then post the file's *contents*: `gh pr comment <n> --body-file <f>` for a PR-level comment, `gh api repos/<owner>/<repo>/pulls/<n>/comments/<comment-id>/replies -F body=@<f>` to reply in a review thread. `-F` is the flag that resolves a leading `@` to file contents; `-f` sets a literal string field, so `-f body=@<f>` posts the path. **Read back what landed.** It fails silently twice over — the post succeeds, and a body carrying no attribution footer exports as a human's (`scripts/gh_export/authorship.py`), so the wreckage returns to the next session as operator guidance nobody has answered — and what sits on the thread meanwhile is a path into your scratchpad, on a repo that may be public. The draft is still on disk, so repair it by PATCHing that comment from the same file rather than deleting and re-posting.
