...
- **A launch prompt that reads like an issue title and ends in `#<N>` is an `/issue` invocation.** Whether it _is_ that issue's title can't be known before the issue is read, and doesn't have to be: a prompt whose whole content is a summary line plus an issue number is the operator handing that issue over, and the missing slash command is a copy artifact. So `@.claude/skills/issue/SKILL.md` runs from its Step 1, whose export settles what the work is — rather than the line being planned as the whole task, a title being a label for the body, comments and attachments that say it.
