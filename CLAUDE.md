...
- **`/lets <task>` hands that judgment to the agent.** An operator who does not want to pre-decide whether the work needs a plan invokes it, and the session picks one of three outcomes: plan and hand off, plan and then implement, or implement with no plan at all. The invocation is a conditional go-ahead scoped to that task and nothing wider: absent it the approval gate is untouched. `@.claude/skills/lets/SKILL.md` owns the two questions that pick the outcome, what each one runs, and which prose forms reach it.
