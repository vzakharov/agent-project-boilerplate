...
- **`scripts/vet.sh` is not optional within G2.** `/finalize`, `/sync-branch` and
  ...
  **rewrite** disposition rather than a choice: there is no version of G2 that
  ...
  `scripts/check-squash-message.sh` is the exception to the rewrite: everything
  else in that file is yours to replace, and a rewrite that drops the call
  silently retires the squash caps at the gate that catches a proposal edited by
  hand or outgrown by a later base merge.
