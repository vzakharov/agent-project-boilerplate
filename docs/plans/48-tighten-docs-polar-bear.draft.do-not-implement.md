...
### The four lenses spell TEND

**Tightness, Existence, Negation, Durability.** The word is the reason the new lens is `negation` rather than `absence`: with `{E, D, T}` fixed by the three shipped names, the fourth letter is the only free one, and `N` is the only choice that yields a word describing what the pass actually does — tending prose that has been left to grow.

`negation` is also the better name on its own merits, acronym aside. It is the issue's own framing (*the sentence survives as its own denial*), and it avoids a real misreading in `absence`: a lens called "absence" sounds like it flags **missing** documentation, which is the opposite of what it finds.

**Two things the mnemonic must not be mistaken for.** It is not the run order — Step 3 resolves existence first, because deciding a line shouldn't exist saves rewording it — and the skill must say so where it names the acronym. And `negation` sits three characters from `narration`, the existing alias for Durability; both stay, since renaming a shipped token to buy separation would cost more than the proximity does, but the single-lens list should present primaries and aliases distinctly enough that the two never have to be told apart at a glance.
...
- **Frontmatter `description`**: extend from three defects to four, name the new one, and carry the `polar bear` idiom. Update the invocation line to `[existence | durability | tightness | negation]`.
- **Intro bullet list** (the three "equal weight" defects): add a fourth bullet — *Negation — text whose subject is a thing the change removed, surviving as its own denial.* Name the TEND mnemonic here, with the not-the-run-order caveat.
- **§ "Single-lens mode"**: add `negation` (alias `polar-bear`) to the token list and the example line; change "all three run" to four.
...
- **§ "Working with skills"**, the `/tighten-docs` bullet: the parenthetical lens list `(existence, durability, tightness)` gains `negation`.
...
- **The TEND mnemonic has one home**, the skill's intro beside the four bullets. `CLAUDE.md`'s lens list stays a bare enumeration — spelling the acronym out in the always-loaded file would be a second copy of a memory aid that costs context on every session and informs none of them.
...
## Resolved: the lens name

`negation` was chosen over `absence`, `imprint`/`imaginary`/`illusion`, and a `permanence`/`restraint` pair, on the acronym plus the misreading argument in § "The four lenses spell TEND". What the rejected options cost: `absence` spells DATE, which is a word but describes nothing the skill does, and invites the "missing documentation" misreading; the `I`-names spell EDIT, the best word available, but every occupant of that slot is either vaguer than `negation` (`imprint`) or implies the prose is inaccurate when its defect is being accurate and useless (`imaginary`, `illusion`); `permanence` + `restraint` spells PARE, whose meaning is exact, but it renames two shipped lenses — one of them the skill's own namesake — to buy an acronym, which is the aesthetics-driven swap CLAUDE.md § "Key principles" rules out.
...
- (b) A named case under Lens A, as the issue leans. Keeps "three equal defects" intact across `CLAUDE.md`, `/go`, `README.md` and the catalog — but needs an escape clause bolted onto Lens A's three-part keep-test, loses single-lens invocability for the operator's `polar bear here` shorthand, buries the count in the `Existence` group, and gives up the TEND mnemonic.
