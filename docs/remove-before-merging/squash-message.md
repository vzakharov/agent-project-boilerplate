...
to check, while a project having one exits 1 until its own checks are
wired in, since over a real stack that exit certifies a run which
verified nothing. `vet.sh` being a `rewrite`, both that exit and the
caps line are the adopter's to decide; the comments above them are the
home, cited from ADOPTING.md and the catalog.
