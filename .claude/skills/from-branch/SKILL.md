...
**`mergeable` and `mergeStateStatus` are fetched as context, not as a work item.** They belong in the attach report, and they are the input `@.claude/skills/handle/SKILL.md` Step 2 reads — nothing here acts on them. Base-merging is `@.claude/skills/finalize/SKILL.md` Step 2's, so a `CONFLICTING` / `DIRTY` head is **reported** and the follow-up dispatched in Step 6 runs regardless. This is the point where the host harness's standing instruction to treat a conflict as work now is loaded and nothing local is arguing the other side, which is why the negative is stated: CLAUDE.md § "Key principles" governs, and the conflict becomes this session's work only on the operator's say-so.
