...
<noreply@anthropic.com>` as the final line (or your other assigned vendor email
  ...
  - **The byline is the last line: no session link after it.** This
    **overrides the harness attribution block** that ends every commit message
    ...
    commits, which the squash discards. A branch here is worked by several
    sessions by design, so the one link a squash record could carry is
    whichever session last touched the proposal, usually the finalize one:
    the session holding least of the reasoning behind the change.
    `scripts/check-squash-message.sh` fails a body that carries one.
