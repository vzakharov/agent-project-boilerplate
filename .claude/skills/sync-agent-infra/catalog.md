...
| `.claude/skills/plainly/voice.md` | The resident short version of that rule — the shape an explanation takes, and the instruction to resolve the session's operator once. CLAUDE.md § "Explaining things to people" imports it, and it imports the operator entries in turn. | — | — | adopt |
| `.claude/skills/plainly/operators.md` | Per-person entries tuning how each operator wants to be talked to, resident by import. Ships carrying this repo's own operator as the worked shape; an entry tunes manner only and can never lower a bar. | — | — | **rewrite** |
...
**§ "Explaining things to people" travels as a pair, and the import line is the
half that is easy to drop.** `voice.md` reaches context only because CLAUDE.md
...
nothing and fails silently. Take the section and the file together. `operators.md`
is **rewrite** rather than adopt: it ships with this repo's own operator as the
worked shape, and yours are different people.
