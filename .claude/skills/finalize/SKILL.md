...
8. **Merge — under `and merge` only, and only if the run was uneventful.** Without the flag there is no step 8: stop at 7 and report.
   ...
   **Stand down if any of this happened**, at whichever step it happened:
   ...
   The list is walked step by step so the verdict is auditable, and it is **not exhaustive** — a procedure that grows a step grows a way for a run to be eventful. The predicate is what governs: **if the run made you decide anything, stand down.** Deciding it well is not the same as the operator having decided it.

   **Merging.** Step 7 goes up first — the attestation is the record of what the merge rests on, and it belongs on the PR while the PR is still where anyone would look. Then squash-merge with the title and body **step 5 settled**, verbatim; do not recompose them here. The comment is the source, so read it back rather than reconstructing it:

   ```bash
   gh api repos/<owner>/<repo>/issues/comments/<id> --jq .body > tmp/proposal.md
   ```

   Split its two fenced blocks into `tmp/squash-title.txt` and `tmp/squash-body.txt`, then **print both and read them** — the title against the `<type>: #<issue> <essence> (pr #<n>)` shape, the body ending at its `Co-authored-by:` byline. A mis-split lands the attribution footer, or the title block, inside a record that is permanent and silent about it. Then:
