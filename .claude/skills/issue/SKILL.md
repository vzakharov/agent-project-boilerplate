---
description: Take a GitHub issue — export and read the thread, split it when the scope demands, then hand the work over to `/plan`. Invoke as `/issue <number|url>`; a prompt that is an issue title ending in `#<N>`, with no slash command, is the same invocation.
---
