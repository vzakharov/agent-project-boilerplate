...
| **A recurrence is not automatically a regression** | Before reopening a closed issue, establish whether the fix was actually *running* when the problem recurred — merged is not deployed. A recurrence while the fix still sits on the trunk is expected, and reopening on it is churn that repeats every run until the release train catches up. Name the two cheap reads that settle it (an ancestry check against whatever ref production ships from; whether any deploy landed inside the window at all), and say what each outcome does — reopen, comment-only, or flag as ambiguous. |
