...
| Item | What it does | Requires | Pulls in | Disposition |
| --- | --- | --- | --- | --- |
| `/task` | Judge for itself whether a task needs the operator's gate, a plan for the agent's own sake, or neither, and run that call: `/task <what to do>` is a conditional go-ahead scoped to that task. The untracked-work sibling of `/issue`. | — | `/go`, `/plan`, `/pr` | adopt |
...
| Item | What it does | Requires | Pulls in | Disposition |
| --- | --- | --- | --- | --- |
| `/issue` | Export and read a GitHub issue, split it into natively-linked sub-issues when the scope demands, then hand the work to `/pr`. | G2, `gh`, `scripts/export-github-item.py` | `/finalize`, `/pr`, `/plan`, `/task` (G2) | adopt |
