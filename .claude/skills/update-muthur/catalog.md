...
| Item | What it does | Requires | Pulls in | Disposition |
| --- | --- | --- | --- | --- |
| `/lets` | Judge for itself whether a task needs the operator's gate, a plan for the agent's own sake, or neither, and run that call: `/lets <task>` is a conditional go-ahead scoped to that task. | — | `/go`, `/plan`, `/pr` | adopt |
...
| `/go` | The go-ahead: flip the plan file, do the work, run the quality passes, hand the PR back to `/pr`. Also takes a branch to attach to, or a task with no plan behind it. | — | `/dry`, `/tend-prose` (G1); `/from-branch`, `/lets`, `/plan`, `/pr` | adopt |
