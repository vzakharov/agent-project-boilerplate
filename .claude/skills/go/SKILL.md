...
2. **`/tend-prose`** — load `@.claude/skills/tend-prose/SKILL.md` and run it over the prose you added. Its lenses carry **equal weight** and it reports a group per lens; the skill enumerates both, and this line deliberately doesn't, having once sat stale through a lens being added. Run the full sweep here rather than naming one. Commit this pass's edits.
